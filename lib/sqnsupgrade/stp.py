...
def start(elf, elfsize, serial, baud=3686400, retry=None, debug=None, AT=True):
    dev = None
    if AT == True:
        init_baud = 115200
    else:
    ...
    try:
        ...
        # 12 is 4096K
        wbits = 12
        dev = SerialDev(serial, init_baud)
    ...
    if AT == True:
        ...
        if data != ok_msg:
            ...
            print("Received: '%s' (%d bytes)"%(data, len(data)))
            print("Early flush data:: '%s' (%d bytes)"%(flush_data, len(flush_data)))
            return
        ...
        if data != ok_msg:
            ...
            print("Received: '%s' (%d bytes)"%(data, len(data)))
            return
        dev.serial.timeout = 60

    print("STP started")
    time.sleep(0.05)
    m = Master(dev, debug=debug)
