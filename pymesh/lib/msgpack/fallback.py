...
def _get_data_from_buffer(obj):
    try:
        ...
        if not PY3:
            ...
            print("using old buffer interface to unpack %s; "
                          "this leads to unpacking errors if slicing is used and "
                          "will be removed in a future version" % type(obj),
                          RuntimeWarning)
        else:
            view = memoryview(bytes([obj]))
            # raise

    # @TODO: not sure what is is for...
    # if view.itemsize != 1:
...
class Unpacker(object):
    ...
    def _unpack(self, execute=EX_CONSTRUCT):
        ...
        if typ == TYPE_RAW:
            if self._encoding is not None:
                ...
                obj = struct.pack("b"*len(obj),*obj).decode('utf8')
    ...
    def skip(self, write_bytes=None):
        ...
        if write_bytes is not None:
            print("`write_bytes` option is deprecated.  Use `.tell()` instead.", DeprecationWarning)
